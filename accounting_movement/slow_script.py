...
from time import perf_counter  # правка

import pandas as pd

print(perf_counter())  # правка
...
for file in sbis_files:
    if 'csv' not in file:
        continue
    df = pd.read_csv(sbis_folder + "/" + file, skiprows=1,
                     sep=";", encoding="windows-1251", header=None)
    dfs.append(df)
...
for file in apteka_files:
    ...
    apteka = pd.read_csv(apteka_folder + file, sep=";", encoding="1251")  # правка
    ...
    apteka.to_excel(
        f"{result_path}{file.split('.csv')[0]} - результат.xlsx", index=False)  # правка
